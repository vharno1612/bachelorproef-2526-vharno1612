...
In dit scenario wordt een abrupte hardware-uitval gesimuleerd door de actieve virtuele machine op kernel-niveau te laten crashen via de Linux Magic SysRq-interface.
...
De gemiddelde RTO van 2,03 seconden bij een volledige node-crash is een opmerkelijk resultaat, aangezien dit sneller is dan de netwerkisolatie scenario 2.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{netwerkschema.png}
    \caption{Uiteindelijk netwerkschema van de geoptimaliseerde Active-Passive HA-omgeving binnen de Azure Resource Group \autocite{Gemini2026}.}
    \label{fig:netwerkschema_final}
\end{figure}
...
De behaalde resultaten overtroffen de initiële verwachtingen. Vooral de vaststelling dat een abrupte Node-level failure sneller herstelt dan een shutdown. Ook dit benadrukt de efficiëntie van de agressieve Corosync-parameters.
\medskip 

Een kritische maar noodzakelijke kanttekening bij deze extreme optimalisaties is dat ze een trade-off met zich meebrengen:
\begin{itemize}
    ...
    \item \textbf{Resourcebelasting}: De belasting van een kort monitor-interval van 2 seconden is voor de nodes en de Load Balancer in de praktijk nihil. De timing is echter cruciaal bij zware database-applicaties: wanneer een database 100\% CPU verbruikt, kan de statuscontrole van Pacemaker vertraging oplopen. Als de check hierdoor buiten het interval van 2 seconden valt, triggert dit een onnodige en foutieve failover. De intervallen moeten dus niet worden afgewogen vanwege de belasting die ze veroorzaken, maar vanwege de belasting die ze moeten tolereren.
\end{itemize}
